...
#else
    qRegisterMetaType<BitcoinUnit>("RikucoinUnit");
#endif
}
...
static const char* qt_argv = "rikucoin-qt";
...
int GuiMain(int argc, char* argv[])
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (invalid_token) {
            InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see Rikucoin-qt -h for a list of options.", argv[i])));
            QMessageBox::critical(nullptr, CLIENT_NAME,
                                  // message cannot be translated because translations have not been initialized
                                  QString::fromStdString("Command line contains unexpected token '%1', see Rikucoin-qt -h for a list of options.").arg(QString::fromStdString(argv[i])));
            return EXIT_FAILURE;
        }
    }
